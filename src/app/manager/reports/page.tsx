...
export default function ReportsPage() {
  ...
  const metrics = [
    { label: "Tips this week", value: "₹62,840", copy: "+12.4% from last week", icon: IndianRupee },
    { label: "Average tip", value: "11.6%", copy: "+0.8 points", icon: TrendingUp },
    { label: "Bills with tips", value: "78%", copy: "214 of 274 bills", icon: ReceiptText },
  ];
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <Card className="gap-5 border-[#ded7ca] bg-white/76 py-5 shadow-none">
        <CardHeader className="flex-row items-center justify-between px-5">
          <div>
            <CardTitle className="text-sm">Daily confirmed tips</CardTitle>
            <p className="mt-1 text-xs text-muted-foreground">Monday–Sunday · Current week</p>
          </div>
          ...
          <div className="flex h-60 items-end gap-3">
            {bars.map((bar) => (
              <div key={bar.day} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                <span className="font-tabular text-[9px] text-muted-foreground">{bar.amount}</span>
                <div className="w-full max-w-16 rounded-t-lg bg-primary/85" style={{ height: `${bar.height}%` }} />
